...
public:
    ...
    inline void Index(Sphere3 *s) {
        ...
        Vector3 perpendicular = Vector3::Cross(toCenter,
                                               Vector3::Cross(toCenter, Vector3(0, 1, 0)));
    ...
    inline void Sort() {
        ...
        median_z = (min_z_spheres[min_z_spheres.size() / 2].min_z +
                    max_z_spheres[max_z_spheres.size() / 2].max_z) / 2.0f;
    }
