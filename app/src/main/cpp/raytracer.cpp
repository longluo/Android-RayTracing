...
void * workerThread(void * ptr){
	...
	for (int y = args.threadNum; y < height; y += NUM_THREADS) {
		...
		for (int x = 0; x < width; x++) {
			...
			} else if(* oldValue >> 24 != 254) {
			    // If the background is not already drawn, draw background.
...
extern "C"
JNIEXPORT void JNICALL Java_me_longluo_raytracing_render_LibRayTracer_initialize(JNIEnv * env, jobject obj, jobject mBitmap) {
...
extern "C"
JNIEXPORT void JNICALL Java_me_longluo_raytracing_render_LibRayTracer_passLightProbe(JNIEnv * env, jobject obj, jobject mBitmap) {
...
extern "C"
JNIEXPORT void JNICALL Java_me_longluo_raytracing_render_LibRayTracer_passBackground(JNIEnv * env, jobject obj, jobject mBitmap) {
...
extern "C"
JNIEXPORT jint JNICALL Java_me_longluo_raytracing_render_LibRayTracer_rayTrace(JNIEnv * env, jobject obj, jobject mBitmap, jlong timeElapsed) {
...
extern "C"
JNIEXPORT void JNICALL Java_me_longluo_raytracing_render_LibRayTracer_setInterlacingEnabled(JNIEnv * env, jobject obj, jboolean enabled) {
	if (enabled) {
		interlace_lines = 2;
	} else {
		interlace_lines = 1;
...
extern "C"
JNIEXPORT void JNICALL Java_me_longluo_raytracing_render_LibRayTracer_setReflectionsEnabled(JNIEnv * env, jobject obj, jboolean enabled) {
	if (scene) {
...
extern "C"
JNIEXPORT void JNICALL Java_me_longluo_raytracing_render_LibRayTracer_setLightprobeEnabled(JNIEnv * env,
...
extern "C"
JNIEXPORT jint JNICALL Java_me_longluo_raytracing_render_LibRayTracer_traceTouch(JNIEnv * env, jobject obj,
                                                                          jfloat x, jfloat y) {
	if (!scene) {
		return -1;
	}
...
extern "C"
JNIEXPORT void JNICALL Java_me_longluo_raytracing_render_LibRayTracer_moveTouch(JNIEnv * env, jobject obj,
