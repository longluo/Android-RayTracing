...
public class Dielectic extends Material {
    ...
    double refraction_idx; //ref_idx是指光密介质的折射指数和光疏介质的折射指数的比值
    ...
    public boolean scatter(Ray r, HitRecord rec, Wrapper wrapper) {
        ...
        if (r.direction().dot(rec.normal) > 0) {
            ...
            ni_over_nt = 1.0f / refraction_idx;
            cosine = -r.direction().dot(rec.normal) / (r.direction().length() * rec.normal.length());      //入射角余弦
        }
    ...
    public boolean refract(Vec3 v, Vec3 n, double nt, Wrapper wrapper) {
        ...
        if (temp > 0.0f) {
            wrapper.refracted = uv.Scale(nt).Add(n.Scale(nt * cos_a1 - Math.sqrt(temp)));
            return true;
        } else {
            return false;
        }
